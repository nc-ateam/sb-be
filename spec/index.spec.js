            ...
            it('Country2-GET responds 200 and specific country of given country- united kingdom', () => {
                ...
                return request
                    ...
                    .then((res) => {
                        ...
                        expect(res.body.country.country).to.equal(
                            'United Kingdom'
                        );
                        expect(res.body.country.geolocation).to.be.an('Array');
                        expect(res.body.country.geolocation.length).to.equal(2);
                        expect(res.body.country.geolocation[0]).to.equal(
                            '0.1278'
                        );
                        expect(res.body.country.geolocation[1]).to.equal(
                            '51.5074'
                        );
                    });
            });
            ...
            it('Country4-GET responds 200 and array of cities is served', () => {
                ...
                return request
                    ...
                    .then((res) => {
                        ...
                        expect(res.body.cities[0].geolocation.length).to.equal(
                            2
                        );
                        expect(res.body.cities[0].geolocation[0]).to.equal(
                            '-2.2426'
                        );
                        expect(res.body.cities[0].geolocation[1]).to.equal(
                            '53.4808'
                        );
                        expect(res.body.cities[0].belongs_to).to.be.an(
                            'Object'
                        );
                        ...
                        expect(
                            res.body.cities[0].belongs_to.geolocation
                        ).to.be.an('Array');
                        expect(
                            res.body.cities[0].belongs_to.geolocation.length
                        ).to.equal(2);
                        expect(
                            res.body.cities[0].belongs_to.geolocation[0]
                        ).to.equal('0.1278');
                        expect(
                            res.body.cities[0].belongs_to.geolocation[1]
                        ).to.equal('51.5074');
                    });
            });
            describe('cities', () => {
                ...
                it('City2-GET responds 200 and specific city of given city- Manchester', () => {
                    ...
                    return request
                        ...
                        .then((res) => {
                            ...
                            expect(res.body.city.geolocation).to.be.an('Array');
                            expect(res.body.city.geolocation.length).to.equal(
                                2
                            );
                            expect(res.body.city.geolocation[0]).to.equal(
                                '-2.2426'
                            );
                            expect(res.body.city.geolocation[1]).to.equal(
                                '53.4808'
                            );
                            expect(
                                res.body.city.belongs_to.geolocation
                            ).to.be.an('Array');
                            expect(
                                res.body.city.belongs_to.geolocation.length
                            ).to.equal(2);
                            expect(
                                res.body.city.belongs_to.geolocation[0]
                            ).to.equal('0.1278');
                            expect(
                                res.body.city.belongs_to.geolocation[1]
                            ).to.equal('51.5074');
                        });
                });
                it('City3-GET responds 200 and all landmarks for given city', () => {
                    ...
                    return request
                        ...
                        .then((res) => {
                            ...
                            expect(res.body.landmarks[0].geolocation).to.be.an(
                                'Object'
                            );
                            expect(
                                res.body.landmarks[0].geolocation.geolocation
                                    .length
                            ).to.equal(2);
                            expect(
                                res.body.landmarks[0].geolocation.geolocation[0]
                            ).to.equal('-2.2309113');
                            expect(
                                res.body.landmarks[0].geolocation.geolocation[1]
                            ).to.equal('53.4774049');
                            expect(res.body.landmarks[0].belongs_to).to.be.an(
                                'Object'
                            );
                            expect(
                                res.body.landmarks[0].belongs_to
                            ).to.contain.keys(
                                'city',
                                'geolocation',
                                'picture_url',
                ...
                it('Landmark1-GET responds 200 and all landmarks', () => {
                    return request
                        ...
                        .then((res) => {
                            ...
                            expect(res.body.landmarks[2].geolocation).to.be.an(
                                'Object'
                            );
                            expect(
                                res.body.landmarks[2].geolocation.geolocation
                                    .length
                            ).to.equal(2);
                            expect(
                                res.body.landmarks[2].geolocation.geolocation[0]
                            ).to.equal('-2.0215345');
                            expect(
                                res.body.landmarks[2].geolocation.geolocation[1]
                            ).to.equal('53.4405305');
                            expect(res.body.landmarks[2].belongs_to).to.be.an(
                                'Object'
                            );
                            expect(
                                res.body.landmarks[2].belongs_to
                            ).to.contain.keys(
                                'city',
                                'geolocation',
                                'picture_url',
